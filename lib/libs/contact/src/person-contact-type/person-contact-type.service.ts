...
export class PersonContactTypeService {
  ...
  async get(personContactTypeId: number) {
    ...
    const result = await this.prismaService.person_contact_type.findUnique({
      where: { id: personContactTypeId },
      include: {
        person_contact_type_locale: true,
      },
    });
    ...
    const locale = result.person_contact_type_locale.reduce(
      (acc, localeData) => {
  ...
  async create(data: CreateDTO) {
    const localeMap = await this.localeService.enabledLocalesMap();
    return this.prismaService.person_contact_type.create({
      data: {
        slug: data.slug,
        created_at: new Date(),
        updated_at: new Date(),
  ...
  async update({ id, data }: { id: number; data: UpdateDTO }) {
    const localeMap = await this.localeService.enabledLocalesMap();
    return this.prismaService.person_contact_type.update({
      where: { id },
      data: {
        slug: data.slug,
        updated_at: new Date(),
