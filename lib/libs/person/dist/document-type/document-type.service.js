...
let DocumentTypeService = class DocumentTypeService {
    ...
    async list(locale, paginationParams) {
        ...
        return this.paginationService.paginate(this.prismaService.person_document_type, paginationParams, {
            where: {
                OR,
            },
            include: {
    ...
    async get(id) {
        const DocumentType = await this.prismaService.person_document_type.findUnique({
            where: { id },
        });
        if (!DocumentType) {
            throw new common_1.NotFoundException(`DocumentType with ID ${id} not found`);
        }
        return DocumentType;
    }
    ...
    async delete({ ids }) {
        if (ids == undefined || ids == null) {
            throw new common_1.BadRequestException(`You must select at least one DocumentType to delete.`);
        }
