...
import { PersonUser } from '@/types/models/PersonUser.ts'
import { usePersonUserDelete } from '@/features/contact/person-user'
import PersonUserCreatePanel from '@/pages/contact/person-user/components/person-user-create-panel'
import PersonUserUpdatePanel from '@/pages/contact/person-user/components/person-user-update-panel'
...
const PersonUpdatePanel = forwardRef(
  ({ data, onUpdated }: PersonUpdatePanelProps, ref) => {
    ...
    const [_, setSelectedItems] = useState<any[]>([])
    ...
    const personUserRef = useRef<any>(null)
    const { mutate: personUserDelete } = usePersonUserDelete()
    const openCreatePersonUser = () => {
      const id = openDialog({
        title: t('create', { ns: 'contact.person-user' }),
        description: t('createText', { ns: 'contact.person-user' }),
        children: () => (
          <PersonUserCreatePanel
            id={Number(data.id)}
            onCreated={() => closeDialog(id)}
          />
        ),
    ...
    const openUpdatePersonUser = (itemPersonUser: PersonUser) => {
      const id = openDialog({
        children: () => (
          <PersonUserUpdatePanel
            id={Number(item?.id)}
            data={itemPersonUser}
            onUpdated={() => closeDialog(id)}
          />
        ),
        title: t('edit', { ns: 'contact.person-user' }),
        description: t('editText', { ns: 'contact.person-user' }),
      })

      return id
    }
    const openDeletePersonUser = (items: PersonUser[]) => {
      return confirm({
        title: `${t('delete', { ns: 'actions' })} ${items.length} ${isPlural(items.length) ? t('items', { ns: 'actions' }) : t('item', { ns: 'actions' })}`,
        description: t('deleteText', { ns: 'contact.person-user' }),
      })
        .then(() =>
          personUserDelete({
            id: Number(data.id),
    ...
    return (
      <TabPanel
        ...
        tabs={[
          ...
          {
            title: t('person_user', { ns: 'modules' }),
            children: (
              <DataPanel
                ref={personUserRef}
                selectable
                ...
                layout='list'
                id={`person-${item?.id}`}
                url={`/person/${item?.id}/person-user`}
                render={(item: PersonUser) => (
                  <div className='flex flex-row gap-2'>
                    <span className='relative px-[0.3rem] py-[0.2rem] text-sm'>
                      {item.id}
                    </span>
                ...
                menuActions={[
                  {
                    ...
                    label: t('edit', { ns: 'actions' }),
                    tooltip: t('editTooltip', { ns: 'contact.person' }),
                    handler: (items: PersonUser[]) => {
                      if (items.length === 1) openUpdatePersonUser(items[0])
                    },
                    show: 'once',
                  },
                  {
