...
import { SubscriptionValueType } from "@/types/models";
...
export function requests() {
  ...
  const subscriptionValueList = async (
    ...
    return request<PaginationResult<SubscriptionValueType>>({
  ...
  const subscriptionValueCreate = async (params: {
    subscriptionId: number;
    data: SubscriptionValueType;
  }) => {
    const { subscriptionId, data } = params;

    return request<SubscriptionValueType>({
      url: `/subscription/${subscriptionId}/subscription-value`,
