...
import { SubscriptionPlanGatewayType } from "@/types/models";
...
export function requests() {
  ...
  const subscriptionPlanGatewayList = async (
    ...
    return request<PaginationResult<SubscriptionPlanGatewayType>>({
  ...
  const subscriptionPlanGatewayCreate = async (params: {
    planId: number;
    data: SubscriptionPlanGatewayType;
  }) => {
    const { planId, data } = params;

    return request<SubscriptionPlanGatewayType>({
      url: `/subscription-plan/${planId}/subscription-plan-gateway`,
