...
import { PaymentNotificationType } from "@/types/models";
...
export function requests() {
  ...
  const paymentNotificationList = async (
    ...
    return request<PaginationResult<PaymentNotificationType>>({
  ...
  const paymentNotificationCreate = async (params: {
    paymentId: number;
    data: PaymentNotificationType;
  }) => {
    const { paymentId, data } = params;

    return request<PaymentNotificationType>({
      url: `/payment/${paymentId}/payment-notification`,
