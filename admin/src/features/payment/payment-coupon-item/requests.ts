...
import { PaymentCouponItemType } from "@/types/models";
...
export function requests() {
  ...
  const paymentCouponItemList = async (
    ...
    return request<PaginationResult<PaymentCouponItemType>>({
  ...
  const paymentCouponItemCreate = async (params: {
    couponId: number;
    data: PaymentCouponItemType;
  }) => {
    const { couponId, data } = params;

    return request<PaymentCouponItemType>({
      url: `/payment-coupon/${couponId}/payment-coupon-item`,
