...
import { clone, HttpClient } from 'isomorphic-git';
...
const cloneBootstrap = (inputs: Input[]) => {
  const dir = inputs.find((input) => input.name === 'name')!.value as string;

  if (!dir) {
    console.error(chalk.red(MESSAGES.INVALID_PROJECT_NAME_ERROR));
  } else {
    clone({
      fs,
      http: http as unknown as HttpClient,
      dir,
      url: 'https://github.com/hed-hog/bootstrap.git',
    });
  }
};
