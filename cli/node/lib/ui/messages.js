...
exports.MESSAGES = {
    ...
    PACKAGE_MANAGER_INSTALLATION_SUCCEED: (name) => name !== '.'
        ? `${emojis_1.EMOJIS.CHECK}  Successfully created project ${chalk.green(name)}`
        : `${emojis_1.EMOJIS.CHECK}  Successfully created a new project`,
    ADD_MODULE_SUCCEED: (name) => name !== '.'
        ? `${emojis_1.EMOJIS.HEDGEHOG}  Successfully added ${chalk.green(name)} module`
        : `${emojis_1.EMOJIS.HEDGEHOG}  Successfully added a new module`,
    CONFIG_DATABASE: `${emojis_1.EMOJIS.CONFIG}  Configure your database connection in ${chalk.green('.env')} file`,
    ...
    HEDHOG_INFORMATION_PACKAGE_MANAGER_FAILED: `${emojis_1.EMOJIS.SMIRK}  cannot read your project package.json file, are you inside your project directory?`,
    HEDHOG_INFORMATION_PACKAGE_WARNING_FAILED: (dependencies) => `${emojis_1.EMOJIS.SMIRK}  failed to compare dependencies versions, please check that following packages are in the same minor version : \n ${dependencies.join('\n')}`,
